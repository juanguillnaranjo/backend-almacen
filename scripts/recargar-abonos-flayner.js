...
// Exactamente 53 abonos del usuario - sin filas extras
const RAW_ABONOS = `2024-10-14,2000000,Abono mano de obra,P2.2.005 - DEUDAS CONTABILIDAD PERSONAL,P1.2.001 - CONSTRUCCION CASA
...
async function recargarAbonos() {
	try {
		...
		if (!cuentaDebeMongo || !cuentaHaberMongo) {
			console.log('[ERROR] Cuentas no encontradas');
			process.exit(1);
		}
		...
		// 4. Limpiar abonos y movimientos previos
		console.log('[LIMPIEZA] Removiendo abonos y movimientos previos...');
		...
		// Limpiar abonos en la factura
		factura.abonos = [];

		console.log('[OK] Previos removidos\n');

		// 5. Parsear y aplicar abonos
		...
		// 8. Guardar proveedor
		console.log('[SAVE] Guardando proveedor...');
